...
tifToTibble <- function(fa){
  ...
  snowPackFiles <- paste0(snowPackPath,list.files(path = snowPackPath,pattern = paste0('-FA-',fa,'.tif')))
  #rasterFiles <- c(conCovFiles,SAPFiles,mostVarsFiles)
  rasterFiles <- snowPackFiles
  
  
  #get field names
  #fieldNames <- c(names(stack(conCovFiles)),names(stack(SAPFiles)),names(stack(mostVarsFiles)))
  #fieldNames <- c(names(stack(snowPackFiles)),names(stack(SAPFiles)),names(stack(mostVarsFiles)))
  #fieldNames[1:72] <- str_extract(string = fieldNames[1:72],pattern = '^.*\\.')
  #names(fieldNames) <- fieldNames
  
  #load raster data into r object
  #r <- stack(rasterFiles)
  r <- raster(snowPackFiles)
  fieldNames <- names(r)
  names(fieldNames) <- fieldNames
  #reset field names
  #names(r) <- fieldNames
...
df <- tibble()
for(i in focalAreasIndex[-51]){
    tmp <- tibble(d[[i]])
    names(tmp)[1] <- "snowPack"
    df <- rbind(df,tmp)
  }

write_csv(x = df, file = paste0(data_path,'snowPack4_2_2022.csv'))
