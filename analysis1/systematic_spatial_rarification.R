...
d <- st_as_sf(x = df6 %>% filter(wilderness == 1) %>% dplyr::select(pixelID,x,y), coords = c("x","y"), crs = 3310)
...
for(dist in dists[2:3]){
  d2 <- spatial_rarify(d,dist)
  spat_rare_pixels <- d2$pixelID
  file_name = paste0("spat_rare_pixels_",as.character(dist),".csv")
