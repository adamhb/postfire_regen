...
traj_fig_df <- df5_filter %>% 
  dplyr::select(pixelID,recoveryTime,burnSev,ARI,RRI,postFirePrecip,postFireMaxT,recovery_rate,mediumReburns,wilderness) %>%
...
traj_fig_nf <- traj_fig_df %>%
  filter(recoveryTime > 0) %>% 
  #filter(pixelID %in% pixels) %>%
  filter(n > 30) %>%
  ggplot(aes(recoveryTime, RRI)) +
  geom_line(aes(group = pixelID),alpha = 0.01, color = "black") +
