...
#CHOOSE MODEL
path_to_mod <- 'analysis4/NF_mods_with_warm_dry/mod118_nf_megram_reduced_1000m__notes_2023-05-04-14-40-04.rda'
#path_to_mod <- 'analysis4/NF_mods_with_warm_dry/mod_minPostFirePrecip212_nf_megram_reduced_1000m_2023-05-09-17-14-32.rda'
load(path_to_mod)
...
makePDF(fig = TwoD_vars_NF,file_name = "2D_vars_NF_POSTFIRE_PRECIP", width = 12, height = 8,units = "in")
makePNG(fig = TwoD_vars_NF,file_name = "2D_vars_NF_POSTFIRE_PRECIP", width = 11, height = 7, units = "in")
...
plot_interaction <- function(x_var,y_var,
  ...
  Fig <- ggplot(ggplot_data,aes_string(x = x_var,
    ...
    scale_x_continuous(limits = c(x_min,x_max),breaks = seq(x_min,x_max,1)) +
    scale_y_continuous(limits = c(y_min,y_max),breaks = seq(y_min,y_max,500)) +
...
NF_interaction_fig <- plot_interaction(x_var = "postFireMaxT",
                                       y_var = "postFirePrecip",
                                       x_axis_label = "Mean daily max temperture [°C] \n of first 3 post-fire years",
                                       y_axis_label = "Mean precipitation [mm yr-1] \n of first 3 post-fire years",
                                       x_min = 12,y_min = 500,
                                       x_max = 24,y_max = 3000,
                                       bin_width = 0.01)
...
makePDF(fig = NF_interaction_fig,file_name = "NF_POSTFIRE_PRECIP_interaction", width = 8, height = 6,units = "in")
makePNG(fig = NF_interaction_fig,file_name = "NF_POSTFIRE_PRECIP_interaction", width = 8, height = 6,units = "in")


get_pct_reduction <- function(var,max_x, min_x){
  preds <- get_fig_data(var,unstandardized_data,mod_pred_data,y_position = 1)
  return( (preds[preds$x == max_x,]$fit - preds[preds$x == min_x,]$fit) /  0.147 )
}

#burn sev reduction
get_pct_reduction("burnSev",3,4)




modelVarsToMake2DPlots
